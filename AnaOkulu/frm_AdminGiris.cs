...
namespace AnaOkulu
{
    public partial class frm_AdminGiris : Form
    {
        ...
        {
            sqlConnection = new SqlConnection(@"Data Source=DESKTOP-M5G8EPH;Initial Catalog=AnaOkulu;Integrated Security=True");
            sqlConnection.Open();

            SqlDataReader read;
            
            if (txt_kullanici.TextLength > 0 && txt_sifre.TextLength > 0)  // textboxlara veri girişi yapılmış mı diye kontrol ediliyor
            {                
                    ...
                    if (read.Read() == true)
                    {
                        ...
                        {
                            MessageBox.Show("Hatalı şifre girildi. Lütfen şifrenizi kontrol ediniz.");
                        }
                    }
                    else
                    {
            ...
            {
                MessageBox.Show("Şifre alanı boş bırakılamaz. Lüften şifre giriniz.");
            }
            else if (txt_kullanici.TextLength == 0 && txt_sifre.TextLength > 0)
            {
                MessageBox.Show("Kullanıcı adı alanı boş bırakılamaz. Lüften kullanıcı adı giriniz");
            }
            else
            {
